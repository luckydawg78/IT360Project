...
def embed_message():
    ...
    try:
        if file_path.lower().endswith('.gif'):
            embed_message_in_gif(file_path, message)
        else:
...
def embed_message_in_gif(file_path, message):
    img = Image.open(file_path)
    frames = []
    for frame in ImageSequence.Iterator(img):
        frame = frame.convert("RGBA")
        frame = lsb.hide(frame, message)
        frames.append(frame)
    output_path = file_path.rsplit('.', 1)[0] + "_stego.gif"
    frames[0].save(output_path, save_all=True, append_images=frames[1:], loop=0)
    analysis_label.config(text=f"Hidden message embedded in GIF. Saved to: {output_path}", bootstyle="success")
